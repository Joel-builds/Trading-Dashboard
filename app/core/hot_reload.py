...
from typing import Callable, Dict, Iterable, Optional, Protocol, TypeVar
...
class _HasModuleHash(Protocol):
    path: str
    module_hash: str


TInfo = TypeVar("TInfo", bound=_HasModuleHash)


class GenericHotReloadWorker(QThread):
    updated = pyqtSignal(list)
    error = pyqtSignal(str)

    def __init__(
        self,
        discover_fn: Callable[[str | Iterable[str]], list[TInfo]],
        watch_paths: str | Iterable[str],
        poll_interval: float = 1.0,
    ) -> None:
        super().__init__()
    ...
    def run(self) -> None:
        while self._running:
            try:
                items = self._discover_fn(self.watch_paths)
                changed = self._detect_changes(items)
                if changed:
    ...
    def _detect_changes(self, items: list[TInfo]) -> bool:
        updated = False
        current: Dict[str, str] = {}
        for info in items:
            current[info.path] = info.module_hash
...
def start_generic_watcher(
    discover_fn: Callable[[str | Iterable[str]], list[TInfo]],
    root_paths: str | Iterable[str],
    on_change: Callable[[list[TInfo]], None],
    on_error: Callable[[str], None],
    poll_interval: float = 1.0,
) -> Optional[GenericHotReloadWorker]:
    worker = GenericHotReloadWorker(discover_fn, root_paths, poll_interval=poll_interval)
